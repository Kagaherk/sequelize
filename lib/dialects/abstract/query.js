...
module.exports = (function() {
  ...
  */
  ...
  var groupJoinData = function(data, includeOptions, options) {
    ...
    if (includeOptions.daoFactory.primaryKeyAttributes.length > 0) {
      primaryKeyAttributes = includeOptions.daoFactory.primaryKeyAttributes
      ...
      if (options.checkExisting) {
        for(var i = 0; i < results.length; i++) {
          if (primaryKeyAttributes && primaryKeyAttributes.length > 0) {
            allPKAttributesMatch = true;
            for(var p = 0; p < primaryKeyAttributes.length; p++) {
              if (results[i][primaryKeyAttributes[p]] !== calleeData[primaryKeyAttributes[p]]) {
                allPKAttributesMatch = false;
                break;
              }
            }
            if (allPKAttributesMatch) {
              existingResult = results[i];
              break;
            }
          }
          else {
            if (Utils._.isEqual(_.omit(results[i], calleeDataIgnore), calleeData)) {
              existingResult = results[i];
              break;
            }
          }
