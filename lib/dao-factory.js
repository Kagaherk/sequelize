  ...
  */
  ...
  DAOFactory.prototype.count = function(options) {
    ...
    return new Utils.CustomEventEmitter(function (emitter) {
      ...
      // For counting, only include required (direct or via descendant) associations
      if (options.include && Array.isArray(options.include) && options.include.length > 0) {
        var keepNeeded = function(includes) {
          return includes.filter(function(include) {
            if (include.include)
              include.include = keepNeeded(include.include);
            return include.required || include.hasIncludeRequired;
          });
        }
        options.include = keepNeeded(options.include);
      }
